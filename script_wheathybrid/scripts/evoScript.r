...
################################################################################
# design by user
my_functions <- c("cost_function", "termination_criteria", "random_numbers")
...
################################################################################
if(use_second_last){
  ### just because we are tracking inbreeding we should remove it)
  parents_list1=parents_list1[,-18]
  parents_list2=parents_list2[,-18]
  results_old=results_old[,-18]
  #################################################################
...
for(index in (n_off1 + 1):n_off){
  ...
  while(valid==FALSE){
    ...
    ################################################################################
    {
      ...
      if(linked_parameter_adjustment){
        ...
        if(linked_parameter_adjustment){
          ...
          if((off[param_link_female[1]] + off [param_link_female[2]] + off[param_link_female[3]] + 
              (round(off[linked_param_female]*0.2))) != (off[linked_param_female])) {
            
            needed_line_f <- off[linked_param_female] - ((round(off[linked_param_female]*0.2))+ off[param_link_female[1]] + off [param_link_female[2]] + off[param_link_female[3]])
            
            samples_f <- randomN(3, abs(needed_line_f))
            off[param_link_female] <- off[param_link_female] + samples_f * (-1)^(needed_line_f<0)
          }
          
          if((off[param_link_male[1]] + off [param_link_male[2]] + off[param_link_male[3]] + 
              (round(off[linked_param_male]*0.2))) != (off[linked_param_male])) {
            
            needed_line_m <- off[linked_param_male] - ((round(off[linked_param_male]*0.2))+ off[param_link_male[1]] + off [param_link_male[2]] + off[param_link_male[3]])
            
            samples_m <- randomN(3, abs(needed_line_m))
            off[param_link_male] <- off[param_link_male] + samples_m* (-1)^(needed_line_m<0)
          }
    ...
    if (all(off[-c(1)] >= 3) && off[10] >= 15 && off[11] >= 15) {
      # here we want to have at least 15 lines in each stage 

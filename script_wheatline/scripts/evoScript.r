...
# Use some high performing candidates from second last pool to don't lose diversity
...
n_off_recombination <- evoParameters$n_off_recombination[sel_tmp_row] + evoParameters$n_off_random[sel_tmp_row]
...
n_off_random <- 0 ## I dont like random!!
...
is_linked = rep(FALSE, config$nfactors)
is_linked[config$linked_parameter_male] = TRUE
is_linked[config$linked_parameter_female] = TRUE
is_linked[config$which_linked_parameter] = TRUE
...
results_new = unique(results_new) ## TP: remove duplicates
################################################################################
...
if(iteration == 2){
  ...
  if(linked_parameter){
    mut_offspring[is_linked] = mut_offspring[is_linked]/2
    mut_parent[is_linked] = mut_parent[is_linked]/2
  }
...
if(use_second_last){
  ...
  if(max(previous_optima) > current_max){
    ...
    if(is.vector(add_optima)){
      add_optima = as.matrix(add_optima)
      add_optima = t(add_optima)
      colnames(add_optima) <- c(column_names_optima, "target_function")
    }
    
    add_optima <- cbind(add_optima, gain = 0, inbreeding = 0)
    
    # Reorder columns to match the desired order
    add_optima <- add_optima[, c("nCrosses", "nDH", "nPYT", "nAYT", "nEYT", 
                                 "newParents_replace", "gain", "inbreeding", "target_function")]
  ...
  while(selected < n_sel_last2){
    if(sum(colSums(t(parents_sofar)==results_old[activ_indi,param_cols])==length(param_cols))==0){
      ...
      if(!(activ_indi > 1 & prod(results_old[activ_indi,] == results_old[activ_indi-1,])==1)){
...
if(iteration > 2){
  ...
  for(index5 in to_check){
    ...
    for(index in which(!is_binary & !is_linked)){
      x_new = x
  ...
  while(valid==FALSE){
    ...
    for(index2 in param_cols[!is_binary]){
      if(rbinom(1,1,mut[index2])==1){
        ...
        mutation_size = sign * runif(1,0,range_mut)
        off[index2] <- off[index2] + mutation_size
...
if (any(off < 3, na.rm = TRUE)) {
  off <- c(490,11,330,252,10,45)
}
    ...
    if(valid){ ## TP: A discussed
      if (off[1] > off[2] && off[3] > off[4] && off[4] > off[5] && off[3] <= 700 && off[6] >= 5 && off[6] <= 50 && off[2] >= 10 && off[5] >= 5 && off[6] >= 1 && off[1] <= 500 ) {
